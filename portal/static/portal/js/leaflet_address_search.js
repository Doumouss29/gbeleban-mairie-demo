    ...
    const SearchControl=L.Control.extend({
      ...
      onAdd:function(){
        ...
        input.addEventListener('input',()=>{
          ...
          const q=input.value.trim();if(q.length<2){closeResults();return;}
          timer=setTimeout(()=>search(false),180);
        });
        ...
        input.addEventListener('blur',()=>{clearTimeout(timer);if(input.value.trim().length>=2)timer=setTimeout(()=>search(true),280);});
        return wrap;
      }
    });
