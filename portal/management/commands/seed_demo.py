...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        SiteSettings.objects.get_or_create(id=1, defaults={
            ...
            "hero_text":"Un territoire d'avenir, une mairie au service de ses citoyens.",
            "mayor_name":"Madame le Maire",
            "mayor_message":"Notre ambition est de rapprocher l'administration municipale des citoyens et de rendre l'action communale plus lisible.",
        })
        ...
        for title, desc, icon, url, order in [
            ("Mes démarches","État civil, autorisations et informations utiles","📄","/pages/demarches/",10),
            ("Plan cadastral","Consulter l'espace urbanisme sécurisé","🗺️","/urbanisme/",20),
            ("Nos projets","Réalisés, en cours et à venir","🏗️","/projets/",30),
            ("Gbéléban en carte","Équipements et points importants","📍","/gbeleban-en-carte/",40),
        ]:
        ...
        for title, category, status, progress, budget in [
            ...
            Project.objects.get_or_create(title=title, defaults={"category":category,"status":status,"progress":progress,"budget":budget,"description":"Donnée de démonstration à remplacer par les informations officielles."})
        News.objects.get_or_create(title="Bienvenue sur le portail numérique de Gbéléban", defaults={"excerpt":"Découvrez les projets, services et informations de la commune.","published_at":date.today()})
        if not MapLayer.objects.exists():
            layer = MapLayer.objects.create(name="Équipements de démonstration", category="Services publics", color="#ef7d00", is_public=True, is_default_visible=True, display_fields=["nom","type","statut"])
            for props, coords in [
