...
def import_layer(uploaded, name, category="", color="#ef7d00", is_public=True):
    data = _load_geojson(uploaded)
    layer = MapLayer.objects.create(name=name, category=category, color=color, is_public=is_public)
    fields = []
...
def import_cadastre(uploaded, fields):
    ...
    for idx, feature in enumerate(data.get("features", []), 1):
        ...
        if area_key and props.get(area_key) not in (None, ""):
            try: area = Decimal(str(props.get(area_key)))
            except InvalidOperation: area = None
        _, was_created = Parcel.objects.update_or_create(reference=reference, defaults={
            "section": val("section_field"), "ilot": val("ilot_field"), "lot": val("lot_field"),
            "parcel_number": val("parcel_field"), "area_m2": area,
            "usage": val("usage_field"), "geometry": geom,
        })
